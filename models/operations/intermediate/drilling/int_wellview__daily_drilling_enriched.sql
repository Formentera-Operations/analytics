...
enriched as (
    select
        ...
        coalesce(tl.time_log_entry_count, 0) as time_log_entry_count,
        coalesce(tl.daily_time_duration_hours, 0) as daily_time_duration_hours,
        coalesce(tl.daily_time_problem_hours, 0) as daily_time_problem_hours,
        coalesce(tl.daily_time_no_problem_hours, 0) as daily_time_no_problem_hours,
        coalesce(tl.daily_on_bottom_minutes, 0) as daily_on_bottom_minutes,
        coalesce(tl.daily_off_bottom_minutes, 0) as daily_off_bottom_minutes,
        coalesce(tl.daily_pipe_moving_minutes, 0) as daily_pipe_moving_minutes,

        coalesce(npt_rn.npt_event_count, npt_dt.npt_event_count, 0) as npt_event_count,
        coalesce(npt_rn.daily_npt_gross_hours, npt_dt.daily_npt_gross_hours, 0) as daily_npt_gross_hours,
        coalesce(npt_rn.daily_npt_net_hours, npt_dt.daily_npt_net_hours, 0) as daily_npt_net_hours,
        coalesce(npt_rn.daily_npt_cost, npt_dt.daily_npt_cost, 0) as daily_npt_cost,
        (
            npt_rn.job_id is null
            and npt_dt.job_id is not null
            and jr.report_number is null
        ) as npt_join_used_date_fallback,

        coalesce(safety_rn.safety_event_count, safety_dt.safety_event_count, 0) as safety_event_count,
        coalesce(safety_rn.safety_check_count, safety_dt.safety_check_count, 0) as safety_check_count,
        coalesce(safety_rn.safety_incident_count, safety_dt.safety_incident_count, 0) as safety_incident_count,
        coalesce(safety_rn.safety_lost_time_hours, safety_dt.safety_lost_time_hours, 0) as safety_lost_time_hours,
        coalesce(safety_rn.safety_estimated_cost, safety_dt.safety_estimated_cost, 0) as safety_estimated_cost,
        (
            safety_rn.job_id is null
            and safety_dt.job_id is not null
            and jr.report_number is null
        ) as safety_join_used_date_fallback,
        ...
        greatest(
            ...
            coalesce(tl.max_time_log_synced_at::timestamp_ntz, '1900-01-01'::timestamp_ntz),
    ...
    left join time_log_agg as tl
        on jr.report_id = tl.job_report_id
