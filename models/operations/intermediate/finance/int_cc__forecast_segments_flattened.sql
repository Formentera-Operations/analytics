...
no_segments as (
    ...
    where 
        ...
        (
            (best_forecast_params is null or best_forecast_params = 'null' or not contains(best_forecast_params::string, 'SEGMENTS'))
            and 
...
enriched as (
    ...
    select
        *,
        ...
        case 
            when forecast_source in ('rate', 'rate_no_segments') then true 
            else false 
        end as is_rate_forecast,
        
        case 
            when forecast_source in ('ratio', 'ratio_no_segments') then true 
            else false 
        end as is_ratio_forecast,
        
        case 
            when segment_data is not null then true 
            else false 
        end as has_segments
