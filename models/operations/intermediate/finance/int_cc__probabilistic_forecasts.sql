...
forecast_scenarios as (
    ...
    select
        ...
        case
            when
                best_forecast_params:P10 is not null
        ...
        try_to_date(regexp_replace(best_forecast_params:SEGMENTS[0]:STARTDATE::string, '[^0-9-]', ''))
            as best_start_date,
        ...
        try_to_date(regexp_replace(best_forecast_params:P10:SEGMENTS[0]:SWDATE::string, '[^0-9-]', ''))
            as p10_switch_date,
        ...
        try_to_date(regexp_replace(best_forecast_params:P50:SEGMENTS[0]:SWDATE::string, '[^0-9-]', ''))
            as p50_switch_date,
        ...
        try_to_date(regexp_replace(best_forecast_params:P90:SEGMENTS[0]:SWDATE::string, '[^0-9-]', ''))
            as p90_switch_date,
...
enriched as (
    ...
    select
        *,
        ...
        case
            when data_frequency = 'MONTHLY' and best_di_nominal > 0
                then
                    (1 - power(1 - best_di_nominal, 12)) * 100
            when data_frequency = 'DAILY' and best_di_nominal > 0
                then
