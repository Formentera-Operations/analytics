...
renamed as (
    select
        ...
        -- partner information
        trim(idrecpartner)::varchar as partner_id,
        trim(idrecpartnertk)::varchar as partner_table,

        -- product and interest
        trim(typfluidprod)::varchar as product_type,
        trim(subtypfluidprod)::varchar as sub_product_type,
        {{ pv_decimal_to_pct('interest') }}::float as interest_pct,
...
enhanced as (
    select
        {{ dbt_utils.generate_surrogate_key(['id_rec']) }} as agreement_partner_sk,
        *,
