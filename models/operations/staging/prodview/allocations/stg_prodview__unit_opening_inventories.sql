...
renamed as (
    select
        ...
        trim(idrecparent)::varchar as parent_opening_inventory_id,
        ...
        -- volumes
        {{ pv_cbm_to_bbl('volhcliq') }}::float as oil_condensate_volume_bbl,
...
enhanced as (
    select
        {{ dbt_utils.generate_surrogate_key(['id_rec']) }} as unit_opening_inventory_sk,
        *,
...
final as (
    select
        unit_opening_inventory_sk,

        -- identifiers
        id_rec,
        parent_opening_inventory_id,
        flow_network_id,
