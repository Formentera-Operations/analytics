...
renamed as (
    select
        ...
        {{ pv_decimal_to_pct('bsw') }}::float as bsw_pct,
        ...
        -- uncorrected volumes
        {{ pv_cbm_to_bbl('voluncorrtotalcalc') }}::float as total_volume_uncorrected_bbl,
        {{ pv_cbm_to_bbl('voluncorrhcliqcalc') }}::float as oil_condensate_volume_uncorrected_bbl,
        {{ pv_cbm_to_bbl('volfreewatercalc') }}::float as free_water_volume_bbl,
        {{ pv_cbm_to_bbl('volsettledsandcalc') }}::float as settled_sand_volume_bbl,
        {{ pv_decimal_to_pct('bswuncorrcalc') }}::float as bsw_uncorrected_pct,
        {{ pv_decimal_to_pct('sandcutuncorrcalc') }}::float as sand_cut_uncorrected_pct,

        -- temperature and pressure corrections
        tempofvol / 0.555555555555556 + 32 as temperature_of_volume_f,
        {{ pv_kpa_to_psi('presofvol') }}::float as pressure_of_volume_psi,
        ...
        {{ pv_cbm_to_bbl('volchgtotalcalc') }}::float as change_in_total_volume_bbl,
        {{ pv_cbm_to_bbl('volchghcliqcalc') }}::float as change_in_oil_condensate_volume_bbl,
        {{ pv_cbm_to_mcf('volchghcliqgaseqcalc') }}::float as change_in_gas_equivalent_oil_cond_volume_mcf,
        {{ pv_cbm_to_bbl('volchgwatercalc') }}::float as change_in_water_volume_bbl,
        {{ pv_cbm_to_bbl('volchgsandcalc') }}::float as change_in_sand_volume_bbl,

        -- final volumes
        {{ pv_cbm_to_bbl('voltotalcalc') }}::float as final_total_fluid_volume_bbl,
        {{ pv_cbm_to_bbl('volhcliqcalc') }}::float as final_hydrocarbon_liquid_volume_bbl,
        {{ pv_cbm_to_mcf('volhcliqgaseqcalc') }}::float as final_gas_equivalent_oil_condensate_volume_mcf,
        ...
        -- capacity
        {{ pv_cbm_to_bbl('volcapacityremaincalc') }}::float as capacity_remaining_bbl,
...
enhanced as (
    select
        {{ dbt_utils.generate_surrogate_key(['id_rec']) }} as tank_reading_sk,
        *,
