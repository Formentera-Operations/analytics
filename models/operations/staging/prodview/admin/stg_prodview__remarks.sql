...
renamed as (
    select
        ...
        trim(typ)::varchar as remark_type,
        trim(com)::varchar as comments,

        -- linked item reference
        trim(idrecitem)::varchar as link_id,
        trim(idrecitemtk)::varchar as link_table,

        -- status flags
        actionrqd::boolean as action_required,
        explainedimbalance::boolean as explains_imbalance,
...
enhanced as (
    select
        {{ dbt_utils.generate_surrogate_key(['id_rec']) }} as remark_sk,
        *,
