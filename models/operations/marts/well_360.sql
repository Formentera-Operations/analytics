...
-- Status standardization lookup (engineer-approved)
status_map as (
    select * from {{ ref('seed_production_status_mapping') }}
),
...
golden_record as (
    select
        ...
        coalesce(
            ...
            case 
                when upper(coalesce(wv.operator_name, pv.operator_name, env.operator)) like '%FORMENTERA%' then true  -- Any Formentera entity = operated
                when coalesce(wv.operator_name, pv.operator_name, env.operator) is not null then false  -- Other operator = non-op
                else null
            end
        ) as is_operated,
        ...
        -- Standardized state (2-char uppercase with name mapping)
        case upper(coalesce(oda.state_code, cc.state, wv.state_province, pv.state_province, env.state_province))
            ...
            -- Already 2-char codes - just uppercase
            else upper(left(coalesce(oda.state_code, cc.state, wv.state_province, pv.state_province, env.state_province), 2))
        end as state,
        ...
        -- Standardized well configuration (normalized case and values)
        case upper(coalesce(wv.well_configuration_type, env.trajectory))
            ...
            when 'SOUTH' then 'UNKNOWN'  -- Data quality issue - direction not config
            when 'UNKNOWN' then 'UNKNOWN'
            else coalesce(upper(coalesce(wv.well_configuration_type, env.trajectory)), 'UNKNOWN')
        end as well_configuration_type,
        ...
        -- Standardized producing method
        case upper(coalesce(pv.producing_method, env.producing_method))
            -- Rod Pump variants
            ...
            when 'PCP' then 'PCP'  -- Progressive Cavity Pump
            when 'BMEG' then 'BMEG'  -- Beam Mechanical Electric Gas
            ...
            when 'SWD' then 'SWD'  -- Saltwater Disposal
            
            -- Status values that shouldn't be here -> null
            when 'PLUGGED & ABANDONED' then null
            when 'TEMP ABANDONED' then null
            when 'SHUT IN' then null  -- Status, not method
            
            -- Other
            when 'NON-OP' then 'NON_OPERATED'
            when 'UNKNOWN' then null
            
            else upper(coalesce(pv.producing_method, env.producing_method))
        end as producing_method,
        ...
        -- Unified status: ProdView (engineer-approved) is primary, then fall back to others
        case
            -- PRIMARY: ProdView status_clean (Parker & engineer approved mapping)
            when pv.prodview_status_clean = 'Producing' then 'PRODUCING'
            when pv.prodview_status_clean = 'Shut In' then 'SHUT_IN'
            when pv.prodview_status_clean = 'Injecting' then 'INJECTING'
            when pv.prodview_status_clean = 'Temp Abandoned' then 'TEMP_ABANDONED'
            when pv.prodview_status_clean = 'Plugged & Abandoned' then 'PLUGGED_ABANDONED'
            when pv.prodview_status_clean = 'Suspended' then 'SHUT_IN'
            when pv.prodview_status_clean = 'Sold' then 'SOLD'
            when pv.prodview_status_clean is not null then upper(replace(pv.prodview_status_clean, ' ', '_'))
            
            -- FALLBACK: ODA status
            when oda.oda_status = 'Producing' then 'PRODUCING'
            when oda.oda_status = 'Shut In' then 'SHUT_IN'
            when oda.oda_status = 'Injector' then 'INJECTING'
            when oda.oda_status = 'Temp Abandoned' then 'TEMP_ABANDONED'
            when oda.oda_status = 'Plugged and Abandoned' then 'PLUGGED_ABANDONED'
            when oda.oda_status = 'Planned' then 'PLANNED'
            when oda.oda_status = 'Drilled' then 'DRILLED'
            when oda.oda_status = 'Completed' then 'COMPLETED'
            when oda.oda_status = 'Sold' then 'SOLD'
            
            -- FALLBACK: Enverus status
            when env.well_status = 'PRODUCING' then 'PRODUCING'
            when env.well_status = 'INACTIVE PRODUCER' then 'SHUT_IN'
            when env.well_status in ('INJECTING', 'INACTIVE INJECTOR') then 'INJECTING'
            when env.well_status = 'TA' then 'TEMP_ABANDONED'
            when env.well_status in ('P & A', 'ABANDONED') then 'PLUGGED_ABANDONED'
            when env.well_status in ('PERMITTED', 'DUC', 'DRILLED') then 'PRE_PRODUCTION'
            
            -- FALLBACK: Combo Curve status
            when cc.combo_curve_status in ('ACT', 'ACTIVE', 'PRODUCING') then 'PRODUCING'
            when cc.combo_curve_status in ('INA', 'SI') then 'SHUT_IN'
            when cc.combo_curve_status = 'INJ' then 'INJECTING'
            when cc.combo_curve_status = 'T&A' then 'TEMP_ABANDONED'
            when cc.combo_curve_status = 'PUD' then 'PUD'
        ...
        s.in_wellview,
        s.in_prodview,
        case when env.api_10 is not null then true else false end as in_enverus,
...
with_lineage as (
    select
        ...
        case 
            when pv.prodview_first_production_date is not null 
                 and (wv.on_production_date is null or pv.prodview_first_production_date <= wv.on_production_date)
                 and (env.first_production_date is null or pv.prodview_first_production_date <= env.first_production_date)
            then 'prodview_daily_volumes'
            when wv.on_production_date is not null 
                 and (pv.prodview_first_production_date is null or wv.on_production_date <= pv.prodview_first_production_date)
                 and (env.first_production_date is null or wv.on_production_date <= env.first_production_date)
            then 'wellview'
            when env.first_production_date is not null 
                 and (pv.prodview_first_production_date is null or env.first_production_date <= pv.prodview_first_production_date)
        ...
        case 
            when cc.is_operated is not null 
                and oda.is_operated is not null 
                and cc.is_operated != oda.is_operated
            then true
            else false
        end as is_operated_conflict,
        
        case 
            when (cc.state is not null and oda.state_code is not null and upper(cc.state) != upper(oda.state_code))
                or (cc.county is not null and oda.county_name is not null and upper(cc.county) != upper(oda.county_name))
            then true
            else false
        end as location_conflict,
        
        -- Enverus vs internal discrepancy flag (informational, not a "conflict")
        case 
            when env.well_name is not null 
                and g.well_name is not null 
                and trim(upper(env.well_name)) != trim(upper(g.well_name))
            then true
            else false
        end as enverus_name_differs,
        
        -- Overall conflict indicator (internal systems only)
        case 
            when (cc.well_name is not null and oda.well_name is not null and trim(upper(cc.well_name)) != trim(upper(oda.well_name)))
                or (cc.api_10 is not null and oda.api_number is not null and cc.api_10 != oda.api_number)
                or (cc.is_operated is not null and oda.is_operated is not null and cc.is_operated != oda.is_operated)
            then true
            else false
        end as has_any_conflict
